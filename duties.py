...
def changelog(ctx: Context) -> None:
    """Update the changelog in-place with latest commits (via semantic-release dry-run)."""
    ctx.run(["semantic-release", "changelog"], title="Updating changelog")
    ctx.run(tools.yore.check(bump=_get_changelog_version()), title="Checking legacy code")


@duty(pre=["check-quality", "check-types", "check-security", "check-deadcode", "check-docs", "check-api"])
...
def check_security(ctx: Context) -> None:
    ...
    ctx.run(
        ["bandit", "-r", *PY_SRC_LIST, "-c", "config/bandit.toml"],
        title=pyprefix("Security check"),
    )


@duty
def check_deadcode(ctx: Context) -> None:
    """Check for dead code."""
    ctx.run(
        ["vulture", *PY_SRC_LIST, "--min-confidence", "80"],
        title=pyprefix("Dead code check"),
    )
...
def check_api(ctx: Context, *cli_args: str) -> None:
    ...
    ctx.run(
        tools.griffe.check("unblu_mcp", search=["src"], color=True).add_args(*cli_args),
